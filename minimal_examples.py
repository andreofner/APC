...
if run_HPC_speech:
  ...
  for plot_b in [True, False]:
    ...
    for updates_xhat in [state_updates]: # 10 paper plot
      ...
      for use_top_down in [False, True]:
        plt_col += 1
        plt_row = -1
        ...
        for update_A_TD in range(updates_A_TD):
          for update_A in range(updates_A):
            ...
            for n in range(data.shape[0] - 1):
              ...
              with tapeKF: MSE_seq = MSE_seq+MSE
          ...
          if update_A_TD % plot_interval == 0:
            if not update_A_TD == 0:
              ...
              axs[plt_row, plt_col].set_ylim([0., 4.])
        ...
        axs[0, 0].title.set_text("Single layer")
        axs[0, 1].title.set_text("Hierarchical")
        ...
        for row in range(4):
          axs[row, 0].xaxis.set_ticklabels([])
          axs[row, 1].xaxis.set_ticklabels([])

        for row in range(5):
          axs[row, 1].yaxis.set_ticklabels([])
          axs[row, 1].set_ylabel('Update ' + str((row+1)*plot_interval))
        ...
        if use_top_down:
          ...
          plt.show()
        """
      f.savefig("./both_"+str(updates_xhat)+"_TDupdates_"+str(updates_A_TD)+"_LRx_"+str(lr_x)+".pdf")
      plt.show()
